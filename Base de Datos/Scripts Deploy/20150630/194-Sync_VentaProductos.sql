/****** Object:  StoredProcedure [dbo].[Sync_VentasProductos]    Script Date: 01/19/2015 17:48:59 ******/
...
BEGIN
	...
	AS (
	 ...
	 UPDATE VP
	 SET VentaId = (SELECT TOP 1 VentaId FROM Venta WHERE Identifier = CTE.VentaIdentifier),  
	...
	;
	 ...
	 SELECT Identifier, 
			...
			Eliminado, 
			Desincronizado,
			(SELECT TOP 1 VentaId FROM Venta WHERE Identifier = CTE.VentaIdentifier),  
