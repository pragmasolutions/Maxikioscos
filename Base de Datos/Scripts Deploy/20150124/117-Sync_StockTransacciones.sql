/****** Object:  StoredProcedure [dbo].[Sync_StockTransacciones]    Script Date: 01/19/2015 17:49:21 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Sync_StockTransacciones]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Sync_StockTransacciones]
GO


CREATE PROCEDURE [dbo].[Sync_StockTransacciones] 
	@XML XML,
...
BEGIN
	...
	AS (
	 ...
	 UPDATE ST
	 SET StockId = (SELECT StockId FROM Stock WHERE Identifier = CTE.StockIdentifier), 
