...
BEGIN
	...
	WITH ControlStockDetalleCTE (Cantidad, FechaUltimaModificacion, 
		Eliminado, Desincronizado, Precio, FechaCorreccion,
	    Correccion, MotivoCorreccionId, Identifier, ControlStockIdentifier, StockIdentifier)
	AS (
		SELECT    *
	 ...
	 SET ControlStockId = (SELECT ControlStockId FROM ControlStock WHERE Identifier = CTE.ControlStockIdentifier), 
		...
		FechaCorreccion = CTE.Desincronizado,
	...
	WHERE (@SobreescribirLocal = 1 OR
	 ...
	 INSERT INTO ControlStockDetalle (Cantidad, FechaUltimaModificacion, 
		Eliminado, Desincronizado, Identifier, ControlStockId, StockId)
	 SELECT Cantidad, 
