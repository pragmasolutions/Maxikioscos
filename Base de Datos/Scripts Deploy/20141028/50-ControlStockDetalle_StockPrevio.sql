/****** Object:  StoredProcedure [dbo].[Sync_ControlesStockDetalle]    Script Date: 10/07/2014 18:36:17 ******/
...
BEGIN
	...
	AS (
	 ...
	 SET ControlStockId = (SELECT ControlStockId FROM ControlStock WHERE Identifier = CTE.ControlStockIdentifier), 
		...
		ControlStockPrevioId = (SELECT ControlStockId FROM ControlStock WHERE Identifier = CTE.ControlStockPrevioIdentifier), 
	...
	WHERE (@SobreescribirLocal = 1 OR
	 ...
	 SELECT CTE.Cantidad, 
		...
		(SELECT ControlStockId FROM ControlStock WHERE Identifier = CTE.ControlStockPrevioIdentifier)
	 FROM @Temp CTE
