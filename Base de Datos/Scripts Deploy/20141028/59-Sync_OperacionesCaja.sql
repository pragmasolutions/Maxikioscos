...
/****** Object:  StoredProcedure [dbo].[Sync_OperacionesCaja]    Script Date: 10/08/2014 18:18:54 ******/
...
BEGIN
	...
	AS (
	 ...
	 SET CierreCajaId = (SELECT CierreCajaId FROM CierreCaja WHERE Identifier = CTE.CierreCajaIdentifier), 
	    UsuarioCreadorId = (SELECT UsuarioId FROM Usuario WHERE Identifier = CTE.UsuarioCreadorIdentifier), 
	    UltimoUsuarioModificacionId = (SELECT UsuarioId FROM Usuario WHERE Identifier = CTE.UsuarioModificacionIdentifier), 
		Monto = CTE.Monto, 
		Fecha = CTE.Fecha, 
	...
	WHERE (@SobreescribirLocal = 1 OR
	 ...
	 SELECT Identifier,
			...
			(SELECT UsuarioId FROM Usuario WHERE Identifier = CTE.UsuarioCreadorIdentifier), 
	        (SELECT UsuarioId FROM Usuario WHERE Identifier = CTE.UsuarioModificacionIdentifier),
			Monto, 
			Fecha, 
			FechaUltimaModificacion, 
			Eliminado, 
