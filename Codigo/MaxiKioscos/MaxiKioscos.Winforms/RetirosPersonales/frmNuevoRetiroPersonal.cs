...
namespace MaxiKioscos.Winforms.RetirosPersonales
{
    ...
    public partial class frmNuevoRetiroPersonal : Form
    {
        #region Repositorios

        private RetiroPersonalRepository _repository;
        public RetiroPersonalRepository Repository
        {
            get { return _repository ?? (_repository = new RetiroPersonalRepository()); }
        }
        ...
        private int? _nroRetiroPersonal;
        private static List<Timer> _timers;

        public frmNuevoRetiroPersonal()
        {
            InitializeComponent();
        ...
        private void ReiniciarRetiroPersonal()
        {
        ...
        {
            if (_nroRetiroPersonal == null)
            {
                _repository = new RetiroPersonalRepository();
                _nroRetiroPersonal = Repository.GenerarNroRetiroPersonal(AppSettings.MaxiKioscoId);
            }
            else
                _nroRetiroPersonal++;
            txtNroRetiroPersonal.Texto = _nroRetiroPersonal.ToString();
        ...
        private void Aceptar(bool sobrescribir = false)
        {
            ...
            {
                //Compruebo que no supere el limite
                if (UsuarioActual.Cuenta.LimiteMaximoRetiroPersonal != null)
                {
                    var total = Convert.ToDecimal(lblTotal.Text.Replace("$", ""));
                    var gastado = Repository.CalcularGastadoEnPeriodo(UsuarioActual.UsuarioId);
                    var maximo = UsuarioActual.Cuenta.LimiteMaximoRetiroPersonal.GetValueOrDefault();

                    if (gastado + total > maximo)
                    {
                        MessageBox.Show("Se ha superado el máximo bisemanal permitido para retiros personales");
                        return;
                    }
                }
                ...
                if (dgvListado.Rows.Count > 0)
                {
                    var lineas = new List<RetiroPersonalProducto>();
                    decimal total = 0;
                    decimal costoTotal = 0;
                    for (int i = 0; i <= dgvListado.Rows.Count - 1; i++)
                    {
                        var linea = new RetiroPersonalProducto();
                    ...
                    var frmConfirmar = new frmConfirmarRetiroPersonal(total);
                    if (frmConfirmar.ShowDialog() == DialogResult.OK)
                    {
                        ConfirmacionAbierta = false;
                        var retiroPersonal = new RetiroPersonal
                        {
                            ImporteTotal = total,
                            ...
                            FechaRetiroPersonal = DateTime.Now,
                            RetiroPersonalProductos = lineas
                        };
                        retiroPersonal.CierreCajaId = UsuarioActual.CierreCajaIdActual;
                        ...
                        var secundarias = new List<RetiroPersonalProducto>();
                        foreach (var linea in lineas.Where(l => l.EsPromocion))
                        {
                            ...
                            secundarias.AddRange(productos.Select(p => new RetiroPersonalProducto()
                                                                       {
                        ...
                        foreach (var line in lineas)
                        {
                            ...
                            else
                            {
                                stock = new Stock()
                                {
                                    ...
                                    OperacionCreacion = "RetiroPersonal en DESKTOP",
