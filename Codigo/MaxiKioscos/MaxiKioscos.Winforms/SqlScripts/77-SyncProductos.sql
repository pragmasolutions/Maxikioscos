ALTER PROCEDURE [dbo].[Sync_Productos] 
	@XML XML,
...
BEGIN
	...
	WITH ProductoCTE (ProductoId, Identifier, Descripcion, PrecioSinIVA, PrecioConIVA,
			...
			AceptaCantidadesDecimales, EsPromocion, RubroIdentifier, MarcaIdentifier, CuentaIdentifier)
	AS (
		SELECT    *
	 ...
	 SET RubroId = (SELECT TOP 1 RubroId FROM Rubro WHERE Identifier = CTE.RubroIdentifier), 
		MarcaId = (SELECT TOP 1 MarcaId FROM Marca WHERE Identifier = CTE.MarcaIdentifier), 
	...
	INSERT INTO Producto (Identifier, RubroId, MarcaId, CuentaId, Descripcion, 
			...
			AceptaCantidadesDecimales, EsPromocion)
	SELECT Identifier, 
