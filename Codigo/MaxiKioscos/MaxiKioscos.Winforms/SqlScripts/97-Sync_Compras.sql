ALTER PROCEDURE [dbo].[Sync_Compras] 
	@XML XML,
...
BEGIN
	...
	WITH CompraCTE (CompraId, Identifier, Fecha, Numero, Desincronizado, FechaUltimaModificacion,
	   ...
	   ImporteFinal, TipoComprobante, FacturaIdentifier, CuentaIdentifier, MaxiKioscoIdentifier)
	AS (
		SELECT    *
	...
	;
	 ...
	 INSERT INTO Compra (Identifier, Fecha, Numero, Desincronizado, FechaUltimaModificacion,
						 ...
						 TotalCompra, Descuento, ImporteFinal, TipoComprobante, FacturaId, CuentaId, MaxiKioscoId)
	 SELECT Identifier, 
			Fecha,
			Numero,
			Desincronizado,
